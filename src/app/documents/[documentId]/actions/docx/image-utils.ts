 ...
 */
export const loadImageViaCanvas = (src: string): Promise<ArrayBuffer | null> => {
 ...
 */
export const fetchImageAsArrayBuffer = async (src: string): Promise<ArrayBuffer | null> => {
  try {
    if (src.startsWith("data:")) {
 ...
 */
export const getActualImageDimensions = (src: string): Promise<{ naturalWidth: number; naturalHeight: number } | null> => {
  return new Promise((resolve) => {
    const img = document.createElement("img");
    img.crossOrigin = "anonymous";
    img.onload = () => {
